...
df_temp <- droplevels(subset(df_pop2, Species %in% as.character(sti_eu$SPECIES[sti_eu$STI > 12.45]))) # 20% hotter dwellers
...
df_temp <- droplevels(subset(df_pop2, Species %in% as.character(sti_eu$SPECIES[sti_eu$STI < 11.30]))) # 20% colder dwellers
...
uaa_country[22,] <- apply(uaa_country[1:17,], 2, function(x){summary(lm(x~c(2000:2016)))$coef[2,1]})/uaa_country[1,]

row.names(uaa_country)[22] <- "d_uaa"
...
input_country[11,] <- area_country[which(area_country$Area %in% names(input_country)),2]*100

input_country <- data.frame(apply(input_country, 2, function(x){x[1:10]/x[11]}))
...
country_data[116,] <- country_data[114,]/country_data[104,]
row.names(country_data)[116] <- "hico_mean_perc"
...
```{r}
# Load data farm area
#https://ec.europa.eu/eurostat/databrowser/view/EF_KVAAREG__custom_4291234/default/table?lang=en

farmsize_country <- read.csv("raw_data/ef_kvaareg__custom_4291424_linear.csv", header = T)

farmsize_country <- dcast(farmsize_country, geo+TIME_PERIOD~agrarea, fun.aggregate=function(y){return(sum(y, na.rm=T))}, value.var="OBS_VALUE")

farmsize_country$bigfarm_perc <- farmsize_country$HA_GE100 + farmsize_country$`HA50-99`

farmsize_country$smallfarm_perc <- farmsize_country$HA_LT2 + farmsize_country$`HA2-4`

farmsize_country$ratiofarm <- farmsize_country$smallfarm_perc/farmsize_country$bigfarm_perc

farmsize_country <- dcast(farmsize_country, TIME_PERIOD~geo, fun.aggregate=function(y){return(sum(y, na.rm=T))}, value.var="ratiofarm")

names(farmsize_country) <- c("Year","Austria","Belgium","Bulgaria","Switzerland","Cyprus","Czech Republic","Germany","Denmark","Estonia","Greece","Spain","Finland","France","Croatia","Hungary","Ireland","Iceland","Italy","Lithuania","Luxembourg","Latvia","Macedonia","Malta","Netherlands","Norway","Poland","Portugal","Romania","Sweden","Slovenia","Slovakia","UK")

row.names(farmsize_country) <- paste0("farmsize",sep="_",farmsize_country$Year)

farmsize_country$Year <- farmsize_country$Macedonia <- NULL
farmsize_country$Malta <- 0

farmsize_country <- farmsize_country[,sort(names(farmsize_country))]


farmsize_country[5,] <- apply(farmsize_country[1:4,], 2, function(x){mean(x, na.rm=T)})
 
row.names(farmsize_country)[5] <- "farmsize_mean"

farmsize_country[6,] <- apply(farmsize_country[1:3,], 2, function(x){summary(lm(x~c(2005,2007,2010)))$coef[2,1]})/farmsize_country[1,]

row.names(farmsize_country)[6] <- "d_farmsize"

country_data <- rbind(country_data, farmsize_country)
...
# Saved data (you can directly use this file to skip the previous steps)
